...
try
{
    ...
    jsonResponse.each { vpg ->
        ...
        def wildAlias = vpg.VpgName
        def vpgDirection = vpg.RecoverySite.type == 'PeerSiteApi' ? 'Outgoing' : 'Incoming'

        def instanceProperties = [
            'zertoappliance.vpgtype' : vpg.VpgType,
            'zertoappliance.vpgdirection' : vpgDirection,
            'zertoappliance.protectedsite' : vpg.ProtectedSiteName,
            'zertoappliance.recoverysite' : vpg.RecoverySiteName,
        ]
